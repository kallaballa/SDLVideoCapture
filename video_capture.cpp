...
VideoCapture* VideoCapture::instance_ = NULL;
...
void VideoCapture::init(size_t width, size_t height, size_t fps, const char* filename, enum AVCodecID codec_id) {
	assert(codec_ == NULL);
...
void VideoCapture::encode(SDL_Surface *surface) {
	if(!initialzed_)
		return;
	assert(codec_ != NULL);
    assert(context_ != NULL);
